...
export function RepoDropdown({ onClick }: { onClick?: () => void }) {
  ...
  useEffect(() => {
    if (!config) return;
    fetch(
      `/api/${config.owner}/${config.repo}/${encodeURIComponent(config.branch)}/site-config/branding?file=logo`,
    )
      .then((r) => r.json())
      .then((data) => {
        if (data?.data?.downloadUrl) setLogoUrl(data.data.downloadUrl);
      })
      .catch(() => {});
  }, [config, config?.owner, config?.repo, config?.branch]);

  // NOTE: Branch switching UI has been temporarily removed.
