...
export default function SermonFeatureStep({ site, onComplete, initialEnabled }: StepProps) {
  ...
  async function handleSubmit() {
    ...
    try {
      if (selection) {
        await saveFeature(site.id, site.slug, "sermons", true);
      } else {
        const result = await completeStep(site.id, "sermons");
