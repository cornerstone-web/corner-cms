...
export default function SeriesFeatureStep({ church, onComplete, initialEnabled }: StepProps) {
  ...
  async function handleSubmit() {
    ...
    try {
      if (selection) {
        await saveFeature(church.id, church.slug, "series", true);
      } else {
        const result = await completeStep(church.id, "series");
    ...
    <div className="space-y-6">
      <div className="space-y-1">
        ...
        <p className="text-muted-foreground text-sm">
          Do you organize your sermons into series?
        </p>
        <p className="text-muted-foreground text-sm">
          Series let you group related sermons together — like a multi-week teaching series or seasonal study.
        </p>
      </div>
      <div className="grid grid-cols-2 gap-3">
        ...
        >
          <p className="font-medium text-sm">Yes, we use sermon series</p>
        </button>
        <button
          type="button"
